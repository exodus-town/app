...
export const ClaimModal = memo<Props>(({ tokenId, onClose }) => {
  ...
  return (
    <Modal open size="tiny" className="ClaimModal">
      ...
      <Modal.Content>
        You are now the owner of the Parcel{" "}
        <div className="coords">
          <i className="pin" /> {toCoords(tokenId).join(",")}
        </div>{" "}
        {!signedMessage && (
          <>
            <br />
            <br />
            You can now sign in to Exodus Town to start editing it.
          </>
        )}
      </Modal.Content>
      <Modal.Actions>
        ...
        >
          {signedMessage ? "Continue" : "Sign In"}
        </Button>
      </Modal.Actions>
    </Modal>
  );
